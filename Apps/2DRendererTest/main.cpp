...
void load_texture(resource::Image const& a_image)
{
    ...
    if (hr != S_OK)
    {
        mLOG_ERR("Fail to create resource for texture");
        return;
    }
    ...
    if (hr != S_OK)
    {
        ...
        mLOG_ERR("Fail to create upload resource for texture");
        return;
    }
...
class RendererTestApp : public m::crossPlatform::IWindowedApplication
{
    void init() override
    {
        ...
        resource::RequestImage request;
        request.path          = "data/textures/Test.png";
        resource::Image image = resource::load_image(request);
